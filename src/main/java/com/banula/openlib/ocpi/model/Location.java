 ...
 * of EVSEs that belong
 * together are installed. Typically, the Location object is the exact location
 * of the group of EVSEs,
 * but it can also be the entrance of a parking garage which contains these
 * EVSEs.
 * The exact way to reach each EVSE can be further specified by its own
 * properties.
 */
@Data
@ToString
...
public class Location {
    ...
    /**
     * Defines if a Location may be published on an website or app etc. When this is
     * set to false, only tokens
     * identified in the field: publish_allowed_to are allowed to be shown this
     * Location.
     * When the same location has EVSEs that may be published and may not be
     * published, two 'Locations' should be created.
     */
    @JsonProperty("publish")
    private Boolean publish;

    /**
     * This field may only be used when the publish field is set to false. Only
     * owners of Tokens that match all the set
    ...
    /**
     * Postal code of the location, may only be omitted when the location has no
     * postal code: in some countries charging
     ...
     * selected by the country_code and party_id of this Location, should be used
     * instead.
     */
     ...
     * location.
     * E.g. when the parking garage closes its barriers over night, is it allowed to
     * charge till the next morning?
     * Default: true
     */
    ...
    /**
     * Timestamp when this Location or one of its EVSEs or Connectors were last
     * updated (or created).
     */
