...
/**
 * CustomExceptionHandler represents an exception that can be thrown to handle custom error scenarios.
 */
@Getter
public class OCPICustomException extends RuntimeException{
    ...
    /**
     * Constructs a CustomExceptionHandler with the given error message, OCPI status, and error code response.
     *
    ...
    public OCPICustomException(String message) {
        this.errorMessage = message;
        this.statusCode = Constants.STATUS_CODE_GENERIC_SERVER_ERROR; //default OCPI status code = 3000, means internal server error
    }
}