...
import static com.banula.ocpi.annotation.processor.AuthorizeHeaders.authorizeRequestHeaders;

/**
 * This class is responsible for authorizing the OCN signature and headers. This notation is to be used only for  methods that will exclusivelly be called by the OCN node, as it will check the OCN signature and headers.
 */
    ...
    public Object processAuthorizationHeader(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if(ocnClient.getConfiguration().isSigningSupported()){
            ...
            if (ocnSignature == null) {
                ...
                throw new OCPICustomException("OCN Signature is missing.", Constants.STATUS_CODE_AUTHORIZATION_EXCEPTION_CODE);
            }
            ...
            if(!isVerified) {
                ...
                throw new OCPICustomException("Invalid OCN Signature.", Constants.STATUS_CODE_AUTHORIZATION_EXCEPTION_CODE);
            }
        }
